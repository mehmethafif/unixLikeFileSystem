...
int init_fs(){
    ...
    root_directory_inode = get_free_inode();
    superBlock.root_dir_inode = root_directory_inode;
    root_dir_block = get_free_block();
    inode[root_directory_inode].type = dir;
    gettimeofday( &( inode[root_directory_inode].last_modified ), NULL );
    inode[root_directory_inode].size = BLOCK_SIZE;
    inode[root_directory_inode].block_count = 1;
    inode[root_directory_inode].direct_block[0] = root_dir_block;

    root_dir->num_entry = 1;
    strncpy(root_dir->dir_entry[0].name, ".", 1 );
    root_dir->dir_entry[0].name[1] = '\0';
    root_dir->dir_entry[0].inode = root_directory_inode;
...
int get_block_size_n_inode_count(int *BLOCK_SIZE_ARG, int *INODE_COUNT_ARG){
    ...
    read_block( 0, (char*) &temp_superblock, sizeof(SuperBlock));
...
int write_file(char *new_name, char *from_name){
    ...
    /// read file, calc size and block_cont
    ...
    //fill the content by creating table n stuf
    int offset = 0;
...
int list_dir_by_inode(int inode_no){
    ...
    for (i = 0; i<MAX_DIR_ENTRY; i++){
        if(  (strlen(dir->dir_entry[i].name) != 0) && ( (strcmp(dir->dir_entry[i].name, ".") !=0) && (strcmp(dir->dir_entry[i].name, "..")!=0) ) ){
            printf("%d \t", dir->dir_entry[i].inode);
            printf("%s \t", dir->dir_entry[i].name);
...
int simple_dumpe2fs(){
    ...
    // print root inode
    printf("%d \t", superBlock.root_dir_inode);
    printf("/ \t");
    printf("directory\t");
